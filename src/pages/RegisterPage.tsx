...
import { register as registerApi, isApiError } from "../service";
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    try {
      const { accessToken, refreshToken } = await registerApi(email, password);
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-8">
        <div className="w-full max-w-[420px] rounded-2xl border border-white/20 bg-white/10 p-8 backdrop-blur-md">
          ...
          <div className="mb-6 flex flex-col items-center">
            ...
            <p className="mt-1 text-sm text-white/70">Присоединяйтесь к нам сегодня</p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleRegister}>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex w-full items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-cyan-400 to-blue-500 py-3 font-semibold text-white shadow-lg shadow-cyan-500/25 transition-opacity hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-50"
            >
